...
def showCalender():
gui = Tk() gui.config(background='light blue')
...
day = day_field.get()
day = datetime.datetime.strptime(day, '%d-%m-
%Y').weekday()
gui_content= calendar.day_name[day]
calday = Label(gui, text= gui_content, font= "Consolas 100 bold") 
calday.grid(row=10, column=4,padx=38)
gui.mainloop()
#Driver code
if _name=='main_':
      new = Tk() new.config(background='#8ED5EC')
      new.title("Calender") new.geometry("950x500")
      cal = Label(new,
      text="Calender",bg='#8ED5EC',font=("times", 28, "bold"))
...
cal.grid(row=1, column=1) year.grid(row=10, column=8)
day.grid(row=10, column=7)
year_field.grid(columnspan=4,padx=10,pady=10, ipadx=20,
ipady=15, row=20,column=8)
day_field.grid(columnspan=4, padx=10,pady=10,ipadx=20,
ipady=15, row=20,column=4)
